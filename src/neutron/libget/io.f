...
C *** We do not need to CALL VAX_TO_LOCAL_INTS() as handled in byte_rel_expn()
                        call byte_rel_expn(
     +                      IBUFFER,ilong,1,IDATA(J),NTC1+1,STATUS)
...
C
	...
	END
C
        SUBROUTINE BYTE_REL_EXPN(IINDATA,NINBY4,NFROM,OUTDATA,NOUT
     *,ISTATUS)
        IMPLICIT NONE
C
...
C   Passed parameters
        INTEGER NIN,NINBY4
        BYTE, dimension(:), allocatable :: INDATA
        integer IINDATA(NINBY4)     
...
c Allocate the byte array, and get the values
      NIN=NINBY4*4
      allocate(indata(NIN))
      jref=0
      do i = 1,NINBY4
...
C       TYPE *,'J=',J,' I=',I
      ...
      deallocate(indata)
        ...
        END
C---------------------------------------------------------------------
        SUBROUTINE GETPARR(RUNID,NAME,RRVALUE,LENGTH_IN,
     1			   LENGTH_OUT,ERRCODE)
        IMPLICIT NONE
C
C *** ERROR CODES ***
...
C
	...
	INTEGER*4, dimension(:), allocatable :: RVALUE
C	REAL*4 RVALUE(LENGTH_IN)
      REAL*4 RRVALUE(LENGTH_IN)
C
C
